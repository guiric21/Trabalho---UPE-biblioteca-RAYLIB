...
bool ChecarColisao(Passaro passaro, Cano canos[], bool isGameOver) {
    for (int i = 0; i < 3; i++) {
        if (canos[i].ativo && !isGameOver) {
            if ((CheckCollisionPointRec(passaro.pontos[0], canos[i].topo) || 
                 CheckCollisionPointRec(passaro.pontos[1], canos[i].topo) || 
                 CheckCollisionPointRec(passaro.pontos[2], canos[i].topo) || 
                 CheckCollisionPointRec(passaro.pontos[0], canos[i].base) || 
                 CheckCollisionPointRec(passaro.pontos[1], canos[i].base) || 
                 CheckCollisionPointRec(passaro.pontos[2], canos[i].base)) &&
